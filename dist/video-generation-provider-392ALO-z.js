...
async function pollRunwayTask(params) {
	for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
		...
		switch (payload.status) {
			...
			case "FAILED":
			case "CANCELLED": throw new Error((typeof payload.failure === "string" ? payload.failure : payload.failure?.message)?.trim() || `Runway video generation ${payload.status.toLowerCase()}`);
			default:
	...
	return {
		...
		async generateVideo(req) {
			...
			try {
				...
				const outputUrls = completed.output?.filter((value) => typeof value === "string" && value.trim());
