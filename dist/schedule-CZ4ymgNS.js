...
import { t as resolveStableNodePath } from "./stable-node-path-C2eU89bk.js";
import { Z as applyPathPrepend } from "./exec-defaults-j-_gGe-l.js";
import { i as parseAbsoluteTimeMs } from "./stagger-DU7FjHYo.js";
...
import { a as isBetaTag, n as DEV_BRANCH, o as isStableTag, r as channelToNpmTag } from "./update-channels-DCZaabwx.js";
import { n as compareSemverStrings, s as detectPackageManager } from "./update-check-Xic-4t3s.js";
...
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
		...
		if (channel === "dev") {
			...
			const preflightRoot = await fs$1.mkdtemp(path.join(os.tmpdir(), "openclaw-update-preflight-"));
			...
			try {
				for (const sha of candidates) {
					...
					const depsStep = await runStep(step(`preflight deps install (${shortSha})`, managerInstallArgs(manager.manager, { compatFallback: manager.fallback && manager.manager === "npm" }), worktreeDir, manager.env));
					steps.push(depsStep);
					if (depsStep.exitCode !== 0) continue;
					const buildStep = await runStep(step(`preflight build (${shortSha})`, managerScriptArgs(manager.manager, "build"), worktreeDir, manager.env));
		...
		try {
			...
			const buildStep = await runStep(step("build", managerScriptArgs(manager.manager, "build"), gitRoot, manager.env));
			steps.push(buildStep);
...
//#endregion
export { readPackageName as _, runGatewayUpdate as a, collectInstalledGlobalPackageErrors as c, detectGlobalInstallManagerForRoot as d, globalInstallArgs as f, normalizePackageTagInput as g, resolveGlobalInstallTarget as h, runChannelPluginStartupMaintenance as i, createGlobalInstallEnv as l, resolveGlobalInstallSpec as m, computeNextRunAtMs as n, canResolveRegistryVersionForPackageTarget as o, resolveExpectedInstalledVersionFromSpec as p, computePreviousRunAtMs as r, cleanupGlobalRenameDirs as s, coerceFiniteScheduleNumber as t, detectGlobalInstallManagerByPresence as u, readPackageVersion as v };
