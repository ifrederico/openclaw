...
import { a as registerAcpRuntimeBackend, s as unregisterAcpRuntimeBackend } from "./manager-D9hIAV3J.js";
...
var AcpxRuntime = class {
	...
	async getManager() {
		...
		if (!this.managerPromise) this.managerPromise = (async () => {
			const manager = this.opts?.managerFactory?.(this.config) ?? new (await (import("./manager-DqnkgI6p.js"))).SessionRuntimeManager(this.config);
	...
	async runProbe() {
		return await (await import("./probe-DyDDSpZX.js")).probeEmbeddedRuntime(this.config);
	}
