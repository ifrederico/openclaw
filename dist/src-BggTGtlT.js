...
import { x as buildMediaPayload } from "./reply-payload-Dp5nBPsr.js";
...
import { r as resolveDefaultGroupPolicy } from "./runtime-group-policy-DxOE0SLn.js";
...
import { r as dispatchReplyFromConfigWithSettledDispatcher } from "./inbound-reply-dispatch-DVuGVGeO.js";
...
import { A as inferPlaceholder, B as safeHostForUrl, D as applyAuthorizationHeaderForUrl, E as IMG_SRC_RE, F as normalizeContentType, I as resolveAttachmentFetchPolicy, L as resolveMediaSsrfPolicy, M as isLikelyImageAttachment, N as isRecord$1, O as extractHtmlFromAttachment, P as isUrlAllowed, R as resolveRequestUrl, S as formatUnknownError, T as GRAPH_ROOT, _ as loadMSTeamsSdkWithAuth, b as classifyMSTeamsSendError, f as resolveMSTeamsCredentials, g as createMSTeamsTokenProvider, h as createMSTeamsAdapter, i as fetchGraphJson, j as isDownloadableAttachment, k as extractInlineImageCandidates, m as createBotFrameworkJwtValidator, w as ATTACHMENT_TAG_RE, x as formatMSTeamsSendErrorHint, y as ensureUserAgentHeader, z as safeFetchWithPolicy } from "./graph-users-d8n2g7lO.js";
import { t as getMSTeamsRuntime } from "./runtime-api-BJidkQW9.js";
import { a as resolveMSTeamsRouteConfig, d as resolveMSTeamsChannelAllowlist, f as resolveMSTeamsUserAllowlist, i as resolveMSTeamsReplyPolicy, n as resolveMSTeamsAllowlistMatch, t as isMSTeamsGroupAllowed } from "./policy-VaGFxn7Q.js";
...
function createMSTeamsReplyDispatcher(params) {
	...
	const { dispatcher, replyOptions, markDispatchIdle: baseMarkDispatchIdle } = core.channel.reply.createReplyDispatcherWithTyping({
		...
		onReplyStart: async () => {
			...
			if (typingIndicatorEnabled && !streamController.hasStream()) await typingCallbacks?.onReplyStart?.();
		},
		typingCallbacks,
		deliver: async (payload) => {
...
async function resolveMSTeamsInboundMedia(params) {
	...
	if (mediaList.length === 0) {
		if (attachments.length > 0 && attachments.some((att) => String(att.contentType ?? "").startsWith("text/html"))) {
	...
	const handleTeamsMessageNow = async (params) => {
		...
		if (activity.replyToId && isChannel && teamId) try {
			...
			const { items: threadMessages } = filterSupplementalContextItems({
				...
				kind: "thread",
				isSenderAllowed: (msg) => groupPolicy === "allowlist" ? resolveMSTeamsAllowlistMatch({
					allowFrom: effectiveGroupAllowFrom,
