...
import { n as setGatewayWsLogStyle } from "./ws-logging-Dfmtj1Mh.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-ARBtYsKu.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-CgDu7pLl.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-BdIRH39m.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-C4VfgaM7.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BlznWDul.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-DoXKiC7c.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-CDezva_8.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-BTgs-Pck.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-BBssIrVW.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-BwOU04nV.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-BhZYtoUp.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-8S4Ykjbj.js");
