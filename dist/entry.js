...
if (!isMainModule({
	...
	const { installGaxiosFetchCompat } = await import("./gaxios-fetch-compat-Dt3uB1Cc.js");
...
function tryHandleRootHelpFastPath(argv, deps = {}) {
	...
	import("./root-help-B3QnaEF6.js").then(({ outputRootHelp }) => {
		outputRootHelp();
	}).catch(handleError);
	return true;
}
function runMainOrRootHelp(argv) {
	...
	import("./run-main-5GWmAbhI.js").then(({ runCli }) => runCli(argv)).catch((error) => {
