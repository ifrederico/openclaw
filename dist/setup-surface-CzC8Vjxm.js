...
import "./session-key-BR3Z-ljs.js";
import { a as hasConfiguredSecretInput } from "./types.secrets-BZdSA8i7.js";
import { D as patchTopLevelChannelConfigSection, P as promptSingleChannelSecretInput, Q as splitSetupEntries, n as buildSingleChannelSecretPromptState, v as mergeAllowFromEntries } from "./setup-wizard-helpers-ecC16ic3.js";
import "./setup-Dp8bIdbL.js";
import { d as resolveFeishuAccount, s as inspectFeishuCredentials, t as probeFeishu, u as resolveDefaultFeishuAccountId } from "./probe-wHoF6aaT.js";
...
function getScopedFeishuConfig(cfg, accountId) {
	const feishuCfg = cfg.channels?.feishu;
	if (accountId === "default") return feishuCfg ?? {};
...
function setFeishuAllowFrom(cfg, accountId, allowFrom) {
	return patchFeishuConfig(cfg, accountId, { allowFrom });
}
function setFeishuGroupPolicy(cfg, accountId, groupPolicy) {
	return patchFeishuConfig(cfg, accountId, { groupPolicy });
}
function setFeishuGroupAllowFrom(cfg, accountId, groupAllowFrom) {
	return patchFeishuConfig(cfg, accountId, { groupAllowFrom });
}
function isFeishuConfigured(cfg, accountId) {
	const feishuCfg = cfg.channels?.feishu ?? {};
	...
	if (resolvedAccountId === "default") return topLevelConfigured;
	const account = feishuCfg.accounts?.[resolvedAccountId];
	if (!account || typeof account !== "object") return topLevelConfigured;
...
async function promptFeishuAllowFrom(params) {
	const existingAllowFrom = resolveFeishuAccount({
		cfg: params.cfg,
...
const feishuSetupWizard = {
	channel,
	resolveAccountIdForConfigure: ({ accountOverride, defaultAccountId }) => normalizeString(accountOverride) ?? defaultAccountId,
	...
	status: {
		...
		resolveStatusLines: async ({ cfg, accountId, configured }) => {
			...
			})() : inspectFeishuCredentials(cfg.channels?.feishu);
	...
	finalize: async ({ cfg, accountId, prompter, options }) => {
		...
		const resolved = resolvedAccount.configured && resolvedAccount.appId && resolvedAccount.appSecret ? {
		...
		const hasConfigCreds = Boolean(typeof scopedConfig.appId === "string" && scopedConfig.appId.trim() && hasConfigSecret);
		...
		if (appSecretResult.action === "use-env") next = patchFeishuConfig(next, resolvedAccountId, {});
		else if (appSecretResult.action === "set") {
			appSecret = appSecretResult.value;
			appSecretProbeValue = appSecretResult.resolvedValue;
			appId = await promptFeishuAppId({
				prompter,
				initialValue: normalizeString(scopedConfig.appId) ?? normalizeString(process.env.FEISHU_APP_ID)
			});
		}
		if (appId && appSecret) {
		...
		const currentMode = resolveFeishuAccount({
			cfg: next,
			accountId: resolvedAccountId
		...
		if (connectionMode === "webhook") {
			...
			if (verificationTokenResult.action === "set") next = patchFeishuConfig(next, resolvedAccountId, { verificationToken: verificationTokenResult.value });
			...
			if (encryptKeyResult.action === "set") next = patchFeishuConfig(next, resolvedAccountId, { encryptKey: encryptKeyResult.value });
		...
		const currentDomain = resolveFeishuAccount({
			cfg: next,
			accountId: resolvedAccountId
		}).domain;
		...
		const groupPolicy = await prompter.select({
			...
			initialValue: resolveFeishuAccount({
				cfg: next,
				accountId: resolvedAccountId
		...
		if (groupPolicy === "allowlist") {
			const existing = resolveFeishuAccount({
				cfg: next,
				accountId: resolvedAccountId
			...
			if (entry) {
				...
				if (parts.length > 0) next = setFeishuGroupAllowFrom(next, resolvedAccountId, parts);
			}
		}
		return { cfg: next };
	},
	dmPolicy: {
		...
		getCurrent: (cfg, accountId) => resolveFeishuAccount({
			cfg,
		...
		promptAllowFrom: async ({ cfg, accountId, prompter }) => await promptFeishuAllowFrom({
			cfg,
			accountId: accountId ?? resolveDefaultFeishuAccountId(cfg),
			prompter
		})
