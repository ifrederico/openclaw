...
import { n as normalizeTalkConfig } from "./talk-umkNj9Rb.js";
...
import { n as listKnownProviderAuthEnvVarNames } from "./provider-env-vars-pk6C_sd4.js";
import { r as listChannelPluginCatalogEntries } from "./catalog-DYcB5w-_.js";
import { t as withBundledPluginAllowlistCompat } from "./bundled-compat-BCabKG5D.js";
import { n as appendAllowedValuesHint, r as summarizeAllowedValues, t as validateJsonSchemaValue } from "./schema-validator-DSVrkbYC.js";
import { t as resolveWebSearchInstallCatalogEntries } from "./web-search-install-catalog-BGLm1UMr.js";
import { c as isWindowsAbsolutePath, i as isAvatarHttpUrl, n as hasAvatarUriScheme, o as isPathWithinRoot, r as isAvatarDataUrl } from "./avatar-policy-B3KBNYI7.js";
import { n as collectConfiguredModelRefs } from "./model-refs-DDcinLNL.js";
import { t as OpenClawSchema } from "./zod-schema-COgITMqv.js";
import { r as shouldWarnOnTouchedVersion } from "./version-6ogXOanN.js";
...
function validateConfigObjectWithPluginsBase(raw, opts) {
	...
	const findInstallableChannelCatalogEntry = (channelId) => {
		const normalizedChannelId = normalizeLowercaseStringOrEmpty(channelId);
		if (!normalizedChannelId) return;
		return listChannelPluginCatalogEntries({
			env: opts.env,
			excludeWorkspace: true
		}).find((entry) => normalizeLowercaseStringOrEmpty(entry.id) === normalizedChannelId);
	};
	...
	if (config.channels && isRecord(config.channels)) for (const key of Object.keys(config.channels)) {
		...
		if (!allowedChannels.has(trimmed)) {
			...
			} else if (hasStalePluginEvidenceForUnknownChannel(trimmed)) warnings.push({
				...issue,
