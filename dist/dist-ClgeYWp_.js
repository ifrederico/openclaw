...
import { t as require_lib$2 } from "./lib-Dth9Atb8.js";
import "./jws-hwqout2g.js";
import { t as require_jsonwebtoken } from "./jsonwebtoken-BBQNhIl7.js";
import "./semver-DkfrdtHe.js";
import { n as require_logging, t as require_http$1 } from "./http-BH65ftYP.js";
import { t as require_dist$2 } from "./dist-DGvbBAq7.js";
import { a as require_events$1, i as require_storage, n as require_asserts, r as require_dist$3, t as require_jwt_validator } from "./jwt-validator-CYK3MbDK.js";
...
var require_dist$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	var i = require_http$1(), b = require_lib$2();
		...
		constructor(t) {
			this.baseUrlRoot = t?.baseUrlRoot ?? _, t ? "request" in t ? this.http = t.clone({
				...
				headers: {
					...
					"User-Agent": "teams.ts[graph]/2.0.5"
				}
...
var require_jwt_validation_middleware = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	function withJwtValidation(params) {
		...
		if (credentials?.clientId) serviceTokenValidator = (0, jwt_validator_1.createServiceTokenValidator)(credentials.clientId, credentials.tenantId, void 0, logger);
		else {
		...
		return async (req, res, next) => {
			...
			if (await serviceTokenValidator.validateAccessToken(authorization, activity.serviceUrl ? { validateServiceUrl: { expectedServiceUrl: activity.serviceUrl } } : void 0)) {
	...
	*/
	function withRemoteFunctionJwtValidation(params) {
		...
		return async (req, res, next) => {
			...
			const tokenPayload = !entraTokenValidator ? null : await entraTokenValidator.validateAccessToken(authToken);
			if (!pageId || !appSessionId || !authToken || !entraTokenValidator || !tokenPayload) {
	...
	*/
	function func(name, cb) {
		...
		this.http.post(`/api/functions/${name}`, (0, middleware_1.withRemoteFunctionJwtValidation)({
			...
			if (!req.context) throw new Error("expected client context");
...
var require_app_events$1 = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	*/
	...
	async function onActivitySent(sender, event) {
		for (const plugin of this.plugins) if (plugin.onActivitySent) await plugin.onActivitySent({
			...event,
			sender
		});
	...
	async function onActivityResponse(sender, event) {
		for (const plugin of this.plugins) if (plugin.onActivityResponse) await plugin.onActivityResponse({
			...event,
			sender
		});
...
var require_event$2 = /* @__PURE__ */ __commonJSMin(((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	...
	function inject(plugin) {
		...
		for (const { key, name } of events) {
			...
			if (name === "error") handler = (event) => {
				...
				return this.onActivity(plugin, event);
			};
	...
	*/
	async function $process(sender, event) {
		...
		for (let i = this.plugins.length - 1; i > -1; i--) {
			...
			if (plugin.onActivity) routes.unshift(async ({ next }) => {
...
//#endregion
//#region node_modules/@microsoft/teams.apps/dist/types/event.js
...
var require_types = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	__exportStar(require_event$1(), exports);
...
//#endregion
//#region node_modules/@microsoft/teams.apps/dist/plugins/http/stream.js
var require_stream = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	var HttpStream = class {
		...
		async close() {
			...
			while (!this.id || this.queue.length) await new Promise((resolve) => setTimeout(resolve, 200));
...
var require_plugin = /* @__PURE__ */ __commonJSMin(((exports) => {
	...
	const teams_api_1 = require_dist$2();
	const $http = __importStar(require_http$1());
	const package_json_1 = __importDefault(require_package());
	const jwt_validation_middleware_1 = require_jwt_validation_middleware();
	...
	*/
	let HttpPlugin = class HttpPlugin {
		...
		*/
		...
		async send(activity, ref) {
			const api = new teams_api_1.Client(ref.serviceUrl, this.client.clone({ token: this.botToken }));
			activity = {
				...activity,
				from: ref.bot,
			...
			if (activity.id) {
				const res = await api.conversations.activities(ref.conversation.id).update(activity.id, activity);
				return {
					...activity,
					...res
				};
		...
		createStream(ref) {
			return new stream_1.HttpStream(new teams_api_1.Client(ref.serviceUrl, this.client.clone({ token: this.botToken })), ref, this.logger);
		}
		/**
		...
		*/
		async onRequest(req, res, _next) {
			const activity = req.body;
			let token;
			if (req.validatedToken) token = req.validatedToken;
			else token = {
				appId: "",
				from: "azure",
				fromId: "",
	...
	__decorate([(0, types_1.Logger)(), __metadata("design:type", Object)], HttpPlugin.prototype, "logger", void 0);
	__decorate([(0, types_1.Dependency)(), __metadata("design:type", $http.Client)], HttpPlugin.prototype, "client", void 0);
	__decorate([(0, types_1.Dependency)(), __metadata("design:type", Object)], HttpPlugin.prototype, "manifest", void 0);
	__decorate([(0, types_1.Dependency)({ optional: true }), __metadata("design:type", Object)], HttpPlugin.prototype, "credentials", void 0);
	__decorate([(0, types_1.Dependency)({ optional: true }), __metadata("design:type", Function)], HttpPlugin.prototype, "botToken", void 0);
	__decorate([(0, types_1.Event)("error"), __metadata("design:type", Function)], HttpPlugin.prototype, "$onError", void 0);
	__decorate([(0, types_1.Event)("activity"), __metadata("design:type", Function)], HttpPlugin.prototype, "$onActivity", void 0);
	...
	var App = class {
		...
		*/
		...
		constructor(options = {}) {
			...
			if (this.credentials?.clientId) this.entraTokenValidator = middleware.createEntraTokenValidator(this.credentials.tenantId || "common", this.credentials.clientId, { logger: this.log });
			const plugins = this.options.plugins || [];
			let httpPlugin = plugins.find((p) => {
				return (0, app_plugins_1.getMetadata)(p).name === "http";
			});
			if (!httpPlugin) {
				httpPlugin = new plugins_1.HttpPlugin(void 0, { skipAuth: this.options.skipAuth });
				plugins.unshift(httpPlugin);
			} else if (this.options.skipAuth) this.log.warn("skipAuth option has no effect when a custom HTTP plugin is provided. Configure authentication on the plugin directly.");
		...
		*/
		async send(conversationId, activity) {
			...
			return await this.http.send((0, teams_api_1.toActivityParams)(activity), ref);
		}
		/**
		* subscribe to an event
		* @param name event to subscribe to
		...
		*/
		...
		async onActivity(sender, event) {
			...
			return await this.process(sender, {
				...event,
				sender
			});
		}
