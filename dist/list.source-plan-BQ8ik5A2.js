...
async function planAllModelListSources(params) {
	...
	const { loadStaticManifestCatalogRowsForList, loadSupplementalManifestCatalogRowsForList } = await import("./list.manifest-catalog-qlkeLIjH.js");
	if (!params.providerFilter) {
		const { loadProviderIndexCatalogRowsForList } = await import("./list.provider-index-catalog-iaMz-Z0B.js");
		return createSourcePlan({
			kind: "registry",
	...
	const { hasProviderStaticCatalogForFilter } = await import("./list.provider-catalog-DEbCheAP.js");
