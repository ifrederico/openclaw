...
import { _ as resolveGroupSessionKey, a as resolveSessionStoreEntry, c as updateSessionStore, m as deriveSessionMetaPatch } from "./store-C1XtW9hV.js";
...
import { t as resolveAndPersistSessionFile } from "./session-file-BJUO5Mr0.js";
...
import { o as getQueueSize, r as clearCommandLane } from "./command-queue-lJ0o3pQC.js";
...
import { n as resolveQueueSettings, t as resolveFastModeState } from "./fast-mode-BbA2K8ts.js";
...
import { n as parseInlineDirectives, t as isDirectiveOnly } from "./directive-handling.parse-1Jf288r8.js";
...
import { n as resolveContextTokens, t as createModelSelectionState } from "./model-selection-g36opjmp.js";
...
function loadCommandsStatus() {
	commandsStatusPromise ??= import("./commands-status.runtime-DpcTYOHn.js");
...
function loadDirectiveImpl() {
	directiveImplPromise ??= import("./directive-handling.impl-ka0rdesC.js");
	return directiveImplPromise;
}
function loadDirectiveFastLane() {
	directiveFastLanePromise ??= import("./directive-handling.fast-lane-BUITj-3N.js");
	return directiveFastLanePromise;
}
function loadDirectivePersist() {
	directivePersistPromise ??= import("./directive-handling.persist.runtime-CV5m1qbf.js");
...
function loadGroupsRuntime() {
	groupsRuntimePromise ??= import("./groups.runtime-CR8IpqKs.js");
...
function loadCommandsRegistry() {
	commandsRegistryPromise ??= import("./commands-registry.runtime-CXch4S7-.js");
	return commandsRegistryPromise;
}
function loadSkillCommands() {
	skillCommandsPromise ??= import("./skill-commands.runtime-DzAZQ8kv.js");
...
async function handleInlineActions(params) {
	...
	const skillCommands = shouldLoadSkillCommands && params.skillCommands ? params.skillCommands : shouldLoadSkillCommands ? (await import("./skill-commands.runtime-DzAZQ8kv.js")).listSkillCommandsForWorkspace({
		workspaceDir,
		cfg,
		agentId,
		skillFilter
	...
	if (skillInvocation) {
		...
		if (dispatch?.kind === "tool") {
			...
			const { createOpenClawTools } = await import("./openclaw-tools.runtime-xZ1emyfY.js");
	...
	if (!isAbortRequestText(command.rawBodyNormalized) && sessionEntry) {
		...
		if (cutoff) await (await import("./abort-cutoff.runtime-CDrn65PF.js")).clearAbortCutoffInSessionRuntime({
			sessionEntry,
			sessionStore,
			sessionKey,
			storePath
	...
	if (handleInlineStatus) {
		const { buildStatusReply } = await import("./commands.runtime-Hav7aGXm.js");
	...
	const runCommands = async (commandInput) => {
		const { handleCommands } = await import("./commands.runtime-Hav7aGXm.js");
...
function loadSessionStoreRuntime$1() {
	sessionStoreRuntimePromise$1 ??= import("./store.runtime-5wA7MnVb.js");
...
function loadPiEmbeddedRuntime() {
	piEmbeddedRuntimePromise ??= import("./pi-embedded.runtime-BIl2NAlc.js");
	return piEmbeddedRuntimePromise;
}
function loadAgentRunnerRuntime() {
	agentRunnerRuntimePromise ??= import("./agent-runner.runtime-Dw3ZT--o.js");
	return agentRunnerRuntimePromise;
}
function loadSessionUpdatesRuntime() {
	sessionUpdatesRuntimePromise ??= import("./session-updates.runtime-BFYmLtP2.js");
	return sessionUpdatesRuntimePromise;
}
function loadSessionStoreRuntime() {
	sessionStoreRuntimePromise ??= import("./store.runtime-5wA7MnVb.js");
...
function loadDeliverRuntime() {
	deliverRuntimePromise ??= import("./deliver-runtime-BbfhKFjb.js");
...
async function forkSessionFromParent(params) {
	return (await import("./session-fork.runtime-Csz4d4Mi.js")).forkSessionFromParentRuntime(params);
}
//#endregion
...
function loadSessionArchiveRuntime() {
	sessionArchiveRuntimePromise ??= import("./session-archive.runtime-HmpZrm9N.js");
...
function loadSessionResetModelRuntime() {
	sessionResetModelRuntimePromise ??= import("./session-reset-model.runtime-x1DN0RUs.js");
	return sessionResetModelRuntimePromise;
}
function loadStageSandboxMediaRuntime() {
	stageSandboxMediaRuntimePromise ??= import("./stage-sandbox-media.runtime-iaKwlaqj.js");
...
async function applyMediaUnderstandingIfNeeded(params) {
	...
	const { applyMediaUnderstanding } = await import("./apply.runtime-DXAPNhpH.js");
	await applyMediaUnderstanding(params);
	return true;
}
async function applyLinkUnderstandingIfNeeded(params) {
	if (!hasLinkCandidate(params.ctx)) return false;
	const { applyLinkUnderstanding } = await import("./apply.runtime-mX1nA2pn.js");
...
async function getReplyFromConfig(ctx, opts, configOverride) {
	...
	const maybeEmitMissingResetHooks = async () => {
		...
		const { emitResetCommandHooks } = await import("./commands-core.runtime-DxlcIRE3.js");
