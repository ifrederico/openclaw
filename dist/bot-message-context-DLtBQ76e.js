...
async function resolveStickerVisionSupport(params) {
	try {
		const { resolveStickerVisionSupportRuntime } = await import("./sticker-vision.runtime-DeMdx6IG.js");
...
async function resolveTelegramInboundBody(params) {
	...
	if (hasAudio && !hasUserText && (!isGroup || requireMention && mentionRegexes.length > 0 && !disableAudioPreflight && senderAllowedForAudioPreflight)) try {
		const { transcribeFirstAudio } = await import("./media-understanding.runtime-D5vBVONQ.js");
...
async function buildTelegramInboundContextPayload(params) {
	...
	const sessionRuntime = await import("./bot-message-context.session.runtime-UNM35PAI.js");
...
async function loadTelegramMessageContextRuntime() {
	telegramMessageContextRuntimePromise ??= import("./bot-message-context.runtime-BfXuykth.js");
