...
import { r as isDeliverableMessageChannel } from "./message-channel-DnQkETjb.js";
import { t as createSubsystemLogger } from "./subsystem-CVf5iEWk.js";
import { t as installProcessWarningFilter } from "./warning-filter-NP4xWPPi.js";
...
import { O as INTERNAL_RUNTIME_CONTEXT_BEGIN, S as sanitizeUserFacingText } from "./errors-By_fjUFz.js";
...
function normalizeRestoredFlowRecord(record) {
	...
	const controllerId = syncMode === "managed" ? normalizeText(record.controllerId) ?? "core/legacy-restored" : void 0;
	return {
		...record,
		syncMode,
...
function normalizeOwnerKey$1(ownerKey) {
...
function assertFlowOwnerKey(ownerKey) {
	const normalized = normalizeOwnerKey$1(ownerKey);
...
function resolveFlowBlockedSummary(task) {
	...
	return task.terminalSummary?.trim() || task.progressSummary?.trim() || void 0;
}
...
function applyFlowPatch(current, patch) {
	const controllerId = patch.controllerId === void 0 ? current.controllerId : normalizeText(patch.controllerId);
	...
	return {
		...
		controllerId,
		currentStep: patch.currentStep === void 0 ? current.currentStep : normalizeText(patch.currentStep),
		blockedTaskId: patch.blockedTaskId === void 0 ? current.blockedTaskId : normalizeText(patch.blockedTaskId),
		blockedSummary: patch.blockedSummary === void 0 ? current.blockedSummary : normalizeText(patch.blockedSummary),
...
function createTaskFlowForTask(params) {
	...
	return createFlowRecord({
		...
		blockedTaskId: terminalFlowStatus === "blocked" ? params.task.taskId.trim() || void 0 : void 0,
...
function setFlowWaiting(params) {
	return updateFlowRecordByIdExpectedRevision({
		...
		patch: {
			status: normalizeText(params.blockedTaskId) || normalizeText(params.blockedSummary) ? "blocked" : "waiting",
...
function assertParentFlowLinkAllowed(params) {
	...
	if (normalizeOwnerKey(flow.ownerKey) !== normalizeOwnerKey(params.ownerKey)) throw new ParentFlowLinkError("owner_key_mismatch", "Task ownerKey must match parent flow ownerKey.", { flowId });
...
function loadTaskRegistryDeliveryRuntime() {
	...
	deliveryRuntimePromise ??= import("./task-registry-delivery-runtime-O1QhZaXM.js");
	return deliveryRuntimePromise;
}
function loadTaskRegistryControlRuntime() {
	controlRuntimePromise ??= import("./task-registry-control.runtime-CCMHV7Ug.js");
...
function normalizeSessionIndexKey(sessionKey) {
...
function getTaskRelatedSessionIndexKeys(task) {
	return [...new Set([normalizeSessionIndexKey(task.ownerKey), normalizeSessionIndexKey(task.childSessionKey)].filter(Boolean))];
}
...
function getTasksByRunScope(params) {
	...
	if (sessionKey) {
		const childMatches = matches.filter((task) => normalizeSessionIndexKey(task.childSessionKey) === sessionKey);
		if (childMatches.length > 0) return childMatches;
		return matches.filter((task) => task.scopeKind === "session" && normalizeSessionIndexKey(task.ownerKey) === sessionKey);
	}
...
function getPeerTasksForDelivery(task) {
	...
	return getTasksByRunId(task.runId).filter((candidate) => candidate.runtime === task.runtime && candidate.scopeKind === task.scopeKind && normalizeComparableText(candidate.ownerKey) === normalizeComparableText(task.ownerKey) && normalizeComparableText(candidate.childSessionKey) === normalizeComparableText(task.childSessionKey));
}
...
function findExistingTaskForCreate(params) {
	...
	const runScopeMatches = runId ? getTasksByRunId(runId).filter((task) => task.runtime === params.runtime && task.scopeKind === params.scopeKind && normalizeComparableText(task.ownerKey) === normalizeComparableText(params.ownerKey) && normalizeComparableText(task.childSessionKey) === normalizeComparableText(params.childSessionKey) && normalizeComparableText(task.parentFlowId) === normalizeComparableText(params.parentFlowId)) : [];
	const exact = runId ? runScopeMatches.find((task) => normalizeComparableText(task.label) === normalizeComparableText(params.label) && normalizeComparableText(task.task) === normalizeComparableText(params.task)) : void 0;
...
function mergeExistingTaskForCreate(existing, params) {
	...
	if (params.preferMetadata) {
		if (nextLabel && normalizeComparableText(existing.label) !== nextLabel) patch.label = nextLabel;
		const nextTask = params.task.trim();
		if (nextTask && normalizeComparableText(existing.task) !== nextTask) patch.task = nextTask;
...
function getLinkedFlowForDelivery(task) {
	...
	const flow = getTaskFlowById(flowId);
	if (!flow) return;
	if (normalizeOwnerKey(flow.ownerKey) !== normalizeOwnerKey(task.ownerKey)) return;
	return flow;
}
...
function updateTask(taskId, patch) {
	...
	const sessionIndexChanged = normalizeSessionIndexKey(current.ownerKey) !== normalizeSessionIndexKey(next.ownerKey) || normalizeSessionIndexKey(current.childSessionKey) !== normalizeSessionIndexKey(next.childSessionKey);
...
function createTaskRecord(params) {
	...
	const record = {
		taskId,
		runtime: params.runtime,
		taskKind: params.taskKind?.trim() || void 0,
		sourceId: params.sourceId?.trim() || void 0,
		requesterSessionKey,
		ownerKey,
		scopeKind,
		childSessionKey: params.childSessionKey,
		parentFlowId: params.parentFlowId?.trim() || void 0,
		parentTaskId: params.parentTaskId?.trim() || void 0,
		agentId: params.agentId?.trim() || void 0,
		runId: params.runId?.trim() || void 0,
...
//#endregion
export { getTaskFlowById as A, formatTaskStatusDetail as B, updateTaskNotifyPolicyById as C, failFlow as D, deleteTaskFlowRecordById as E, resolveTaskFlowForLookupToken as F, sanitizeTaskStatusText as H, resumeFlow as I, setFlowWaiting as L, listTaskFlowRecords as M, listTaskFlowsForOwnerKey as N, findLatestTaskFlowForOwnerKey as O, requestFlowCancel as P, updateFlowRecordByIdExpectedRevision as R, setTaskRunDeliveryStatusByRunId as S, createTaskFlowForTask as T, formatTaskStatusTitle as V, markTaskTerminalByRunId as _, findTaskByRunId as a, resolveTaskForLookupToken as b, linkTaskToFlowById as c, listTasksForFlowId as d, listTasksForOwnerKey as f, markTaskRunningByRunId as g, markTaskLostById as h, ensureTaskRegistryReady as i, getTaskFlowRegistryRestoreFailure as j, finishFlow as k, listTaskRecords as l, listTasksForSessionKey as m, createTaskRecord as n, getTaskById as o, listTasksForRelatedSessionKey as p, deleteTaskRecordById as r, isParentFlowLinkError as s, cancelTaskById as t, listTasksForAgentId as u, maybeDeliverTaskTerminalUpdate as v, createManagedTaskFlow as w, setTaskCleanupAfterById as x, recordTaskProgressByRunId as y, buildTaskStatusSnapshot as z };
