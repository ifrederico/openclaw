import { I as resolveMemoryDreamingWorkspaces, L as resolveMemoryLightDreamingConfig, N as resolveMemoryCorePluginConfig, P as resolveMemoryDeepDreamingConfig, R as resolveMemoryRemDreamingConfig, j as formatMemoryDreamingDay } from "./dreaming-BXqo2sMj.js";
import "./memory-core-host-status-BJj-fXlX.js";
import { a as readShortTermRecallEntries, c as repairShortTermPromotionArtifacts, i as rankShortTermPromotionCandidates, n as applyShortTermPromotions, o as recordDreamingPhaseSignals, s as recordShortTermRecalls } from "./short-term-promotion-_GJfb-w4.js";
...
const DREAMS_FILENAME_ALIASES = [PRIMARY_DREAMS_FILENAME, "dreams.md"];
...
async function resolveDreamsPath$1(workspaceDir) {
...
async function writeInlineDeepDreamingBlock(params) {
	const inlinePath = await resolveDreamsPath$1(params.workspaceDir);
	await fs.mkdir(path.dirname(inlinePath), { recursive: true });
	const updated = replaceManagedBlock({
		original: await fs.readFile(inlinePath, "utf-8").catch((err) => {
...
async function writeDailyDreamingPhaseBlock(params) {
	...
	if (shouldWriteInline(params.storage)) {
		inlinePath = await resolveDreamsPath$1(params.workspaceDir);
		await fs.mkdir(path.dirname(inlinePath), { recursive: true });
...
async function appendNarrativeEntry(params) {
	...
	if (existing.includes(DIARY_START_MARKER) && existing.includes(DIARY_END_MARKER)) {
		...
		if (existing.trim().length === 0) updated = diarySection;
		else updated = diarySection + "\n" + existing;
	}
...
async function generateAndAppendDreamNarrative(params) {
	...
	try {
		...
		params.logger.warn(`memory-core: narrative generation failed for ${params.data.phase} phase: ${err instanceof Error ? err.message : String(err)}`);
	} finally {
		try {
...
function normalizeDailyIngestionState(raw) {
	const filesRaw = asRecord$1(asRecord$1(raw)?.files);
	if (!filesRaw) return {
		version: 1,
		files: {}
	};
...
async function collectDailyIngestionBatches(params) {
	...
	for (const file of files) {
		...
		if (!raw) continue;
		const chunks = buildDailySnippetChunks(raw.split(/\r?\n/), perFileCap);
		const results = [];
...
function buildManagedDreamingPatch(job, desired) {
	...
	if (normalizeTrimmedString(job.sessionTarget)?.toLowerCase() !== "main") patch.sessionTarget = "main";
	if (normalizeTrimmedString(job.wakeMode)?.toLowerCase() !== "next-heartbeat") patch.wakeMode = "next-heartbeat";
	const payloadKind = normalizeTrimmedString(job.payload?.kind)?.toLowerCase();
	const payloadText = normalizeTrimmedString(job.payload?.text);
	if (payloadKind !== "systemevent" || !compareOptionalStrings(payloadText, desired.payload.text)) patch.payload = desired.payload;
...
function resolveCronServiceFromStartupEvent(event) {
...
async function reconcileShortTermDreamingCronJob(params) {
	...
	let removedLegacy = 0;
	for (const job of legacyPhaseJobs) try {
		if ((await cron.remove(job.id)).removed === true) removedLegacy += 1;
	} catch (err) {
		params.logger.warn(`memory-core: failed to remove legacy managed dreaming cron job ${job.id}: ${formatErrorMessage(err)}`);
	}
	if (removedLegacy > 0) params.logger.info(`memory-core: removed ${removedLegacy} legacy phase dreaming cron job(s).`);
...
async function runShortTermDreamingPromotionIfTriggered(params) {
	if (params.trigger !== "heartbeat") return;
	if (params.cleanedBody.trim() !== DREAMING_SYSTEM_EVENT_TEXT) return;
...
function registerShortTermPromotionDreaming(api) {
	api.registerHook("gateway:startup", async (event) => {
		try {
			...
			if (!cron && config.enabled) api.logger.warn("memory-core: managed dreaming cron could not be reconciled (cron service unavailable).");
...
//#endregion
export { resolveShortTermPromotionDreamingConfig as n, previewRemDreaming as r, registerShortTermPromotionDreaming as t };
