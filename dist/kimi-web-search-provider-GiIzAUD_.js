...
import { n as MOONSHOT_CN_BASE_URL, o as isNativeMoonshotBaseUrl, r as MOONSHOT_DEFAULT_MODEL_ID, t as MOONSHOT_BASE_URL } from "./provider-catalog-HDaJJZi4.js";
...
function resolveKimiModel(kimi) {
	return (typeof kimi?.model === "string" ? kimi.model.trim() : "") || DEFAULT_KIMI_SEARCH_MODEL;
}
...
function createKimiToolDefinition(searchConfig, openClawConfig) {
	return {
...
async function runKimiSearchProviderSetup(ctx) {
	...
	const normalizedBaseUrl = (typeof existingPluginConfig?.baseUrl === "string" ? existingPluginConfig.baseUrl.trim() : "").replace(/\/+$/, "");
	const existingModel = typeof existingPluginConfig?.model === "string" ? existingPluginConfig.model.trim() : "";
...
function createKimiWebSearchProvider() {
	return {
		...
		createTool: (ctx) => createKimiToolDefinition(mergeScopedSearchConfig(ctx.searchConfig, "kimi", resolveProviderWebSearchPluginConfig(ctx.config, "moonshot")), ctx.config)
	};
}
const __testing = {
	resolveKimiApiKey,
