import { Do as defaultRuntime, Ga as resolveUserPath, L as writeConfigFile, M as readConfigFileSnapshot } from "./env-D1ktUnAV.js";
...
import { n as buildPluginCompatibilityNotices, o as formatPluginCompatibilityNotice } from "./status-DqeSTTLa.js";
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-WW10vZjs.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-C34yzqTe.js");
		const { logConfigUpdated } = await import("./logging-kXnS9ssH.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-Cb5mavxz.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BLBzQyOW.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-ZXkcJaOz.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-P3KdP65d.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-DPuJGpSo.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-uf0UKHwk.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DREIT2ko.js");
		const { setupChannels } = await import("./onboard-channels-BOFQ1YJ-.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-Bvu-CPTI.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-BVnN_2eH.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-CahCOtUx.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-D4UNHBYc.js");
