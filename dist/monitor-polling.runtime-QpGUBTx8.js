...
import { C as withTelegramApiErrorLogging, k as isRecoverableTelegramNetworkError } from "./send-sxDwUGaO.js";
import { t as TELEGRAM_GET_UPDATES_REQUEST_TIMEOUT_MS } from "./request-timeouts-GoE7QdJG.js";
import { n as readTelegramUpdateOffset, r as writeTelegramUpdateOffset } from "./update-offset-store-DbzxmGI0.js";
import { t as createTelegramBot } from "./bot-Ce301bOE.js";
...
var TelegramPollingLivenessTracker = class {
	...
	detectStall(params) {
		...
		if (stall.elapsedMs <= params.thresholdMs) return null;
		if (this.#stallDiagLoggedAt && now - this.#stallDiagLoggedAt < params.thresholdMs / 2) return null;
		this.#stallDiagLoggedAt = now;
		return { message: `Polling stall detected (${stall.label}); forcing restart. [diag ${this.formatDiagnosticFields("error")} apiElapsedMs=${stall.apiElapsedMs}]` };
	}
	...
	#resolvePollingStallSnapshot(now) {
		...
		return {
			elapsedMs,
			apiElapsedMs: now - (this.#latestInFlightApiStartedAt == null ? this.#lastApiActivityAt : Math.max(this.#lastApiActivityAt, this.#latestInFlightApiStartedAt)),
			label: this.#inFlightGetUpdates > 0 ? `active getUpdates stuck for ${formatDurationPrecise(elapsedMs)}` : `no completed getUpdates for ${formatDurationPrecise(elapsedMs)}`
		};
	}
	#now() {
