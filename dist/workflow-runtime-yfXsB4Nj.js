...
import { o as normalizeBaseUrl } from "./provider-request-config-BRDGCVdB.js";
...
import { l as ssrfPolicyFromDangerouslyAllowPrivateNetwork, n as buildHostnameAllowlistPolicyFromSuffixAllowlist } from "./ssrf-policy-Cb9w9jMO.js";
import "./ssrf-runtime-DGIvmaoK.js";
import { t as isProviderApiKeyConfigured } from "./provider-auth-KAwVk4Dy.js";
...
function resolveComfyWorkflowSource(config) {
	...
	return { workflowPath: readConfigString(config, "workflowPath") };
}
...
async function uploadInputImage(params) {
	...
	form.set("image", new Blob([toBlobBytes(params.image.buffer)], { type: params.image.mimeType }), params.image.fileName?.trim() || `input.${inferFileExtension({ mimeType: params.image.mimeType })}`);
	form.set("type", "input");
	...
	try {
		if (params.mode === "cloud" && [
			...
			try {
				...
				const mimeType = redirected.response.headers.get("content-type")?.trim() || "application/octet-stream";
				return {
		...
		const mimeType = firstResponse.response.headers.get("content-type")?.trim() || "application/octet-stream";
		return {
...
function isComfyCapabilityConfigured(params) {
	...
	const hasWorkflow = Boolean(resolveComfyWorkflowSource(capabilityConfig).workflow || readConfigString(capabilityConfig, "workflowPath"));
...
async function runComfyWorkflow(params) {
	...
	const promptInputName = readConfigString(capabilityConfig, "promptInputName") ?? DEFAULT_PROMPT_INPUT_NAME;
	const inputImageNodeId = readConfigString(capabilityConfig, "inputImageNodeId");
	const inputImageInputName = readConfigString(capabilityConfig, "inputImageInputName") ?? DEFAULT_INPUT_IMAGE_INPUT_NAME;
	...
	const providerModel = params.model?.trim() || "workflow";
	setWorkflowInput({
		workflow,
		nodeId: promptNodeId,
	...
	const promptId = (await readJsonResponse({
