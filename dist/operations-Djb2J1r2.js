...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-L_50cWUi.js");
	if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
	...
	const { loadCrestodianOverview } = await import("./overview-BQdFK_x0.js");
...
async function formatOverviewForOperation(overview, deps) {
	...
	const { formatCrestodianOverview } = await import("./overview-BQdFK_x0.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await import("./shared-DUvNENWi.js")).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "config-set") {
		...
		await (opts.deps?.runConfigSet ?? (async (setOpts) => {
			const { runConfigSet: importedRunConfigSet } = await import("./config-cli-EOyEXYW5.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-CDLzomNY.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await import("./doctor-CMajxgZe.js")).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await import("./config/config.js");
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await import("./doctor-CMajxgZe.js")).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-Dl4cpJbr.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-B4UZ7nF9.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
	...
	if (operation.kind === "set-default-model") {
		...
		const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
		const before = await readConfigFileSnapshot();
		const { applyDefaultModelPrimaryUpdate } = await import("./shared-DUvNENWi.js");
		const result = await mutateConfigFile({
			base: "source",
			mutate: (cfg) => {
