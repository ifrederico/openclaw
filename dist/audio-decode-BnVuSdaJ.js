...
function isMp3(buf) {
	...
	return buf[0] === 73 && buf[1] === 68 && buf[2] === 51 || buf[0] === 255 && (buf[1] & 224) === 224 || buf[0] === 84 && buf[1] === 65 && buf[2] === 71;
}
function isWav(buf) {
...
const decoders = {
	async oga(buf) {
		...
		if (!decoder) {
			let { OggVorbisDecoder } = await import("./ogg-vorbis-DVWDb-w0.js");
	...
	async mp3(buf) {
		...
		if (!decoder) {
			const { MPEGDecoder } = await import("./mpg123-decoder-BIB9OPVf.js");
	...
	async flac(buf) {
		...
		if (!decoder) {
			const { FLACDecoder } = await import("./flac-uhzaf4CQ.js");
	...
	async opus(buf) {
		...
		if (!decoder) {
			const { OggOpusDecoder } = await import("./ogg-opus-decoder-DfqXc5CK.js");
	...
	async wav(buf) {
		...
		if (!decode) {
			let module = await import("./node-wav-C4wpojm8.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
	...
	async qoa(buf) {
		...
		if (!decode) decoders.qoa.decode = decode = (await import("./qoa-format-nxpo-kFA.js")).decode;
		return buf && createBuffer(await decode(buf));
	}
};
