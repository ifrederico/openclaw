...
import { a as createWebhookInFlightLimiter, t as WEBHOOK_BODY_READ_DEFAULTS } from "./webhook-request-guards-uO5Z6rny.js";
import "./webhook-ingress-BwX4VVop.js";
import { t as zod_exports } from "./zod-COH8D-AP.js";
import "./api-Zs9KuZcM.js";
import { a as deepMergeDefined, i as validateProviderConfig, n as normalizeVoiceCallConfig, r as resolveVoiceCallConfig } from "./config-Cg78eY5t.js";
import { n as mapVoiceToPolly, t as escapeXml } from "./voice-mapping-CGp4KnVb.js";
import { a as convertPcmToMulaw8k, t as isProviderStatusTerminal } from "./call-status-iUc4W1Ql.js";
import { t as getHeader } from "./http-headers-lhiYgSRu.js";
...
function resolveProviderVoiceSetting(providerConfig) {
	...
	if (typeof candidate.voice === "string" && candidate.voice.trim()) return candidate.voice;
	if (typeof candidate.voiceId === "string" && candidate.voiceId.trim()) return candidate.voiceId;
}
...
function selectConfiguredOrAutoProvider(params) {
	const configuredProviderId = params.configuredProviderId?.trim() || void 0;
	...
	return {
		...
		provider: configuredProvider ?? [...params.listProviders()].sort((left, right) => (left.autoSelectOrder ?? Number.MAX_SAFE_INTEGER) - (right.autoSelectOrder ?? Number.MAX_SAFE_INTEGER))[0]
	};
}
...
var VoiceCallWebhookServer = class {
	...
	*/
	async initializeMediaStreaming() {
		...
		const { getRealtimeTranscriptionProvider, listRealtimeTranscriptionProviders } = await import("./realtime-transcription.runtime-z3xuN9lB.js");
  ...
  <Play loop="0">https://s3.amazonaws.com/com.twilio.music.classical/BusyStrings.mp3</Play>
	...
	*/
	async handleInboundResponse(callId, userMessage) {
		...
		try {
			const { generateVoiceResponse } = await import("./response-generator-iNYp1aL7.js");
...
async function resolveProvider(config) {
	...
	switch (config.provider) {
		case "telnyx": {
			const { TelnyxProvider } = await import("./telnyx-BOfNWslB.js");
		...
		case "twilio": {
			const { TwilioProvider } = await import("./twilio-xkYA0Tt1.js");
		...
		case "plivo": {
			const { PlivoProvider } = await import("./plivo-74qCZiAo.js");
		...
		case "mock": {
			const { MockProvider } = await import("./mock-kJGY-cc_.js");
...
async function resolveRealtimeProvider(params) {
	const { getRealtimeVoiceProvider, listRealtimeVoiceProviders } = await import("./realtime-voice.runtime-BnhMDSkN.js");
	const resolution = resolveConfiguredCapabilityProvider({
...
async function createVoiceCallRuntime(params) {
	...
	if (realtimeProvider) {
		const { RealtimeCallHandler } = await import("./realtime-handler-B4hL9yMU.js");
