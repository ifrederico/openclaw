...
import { f as isPathInside, i as openBoundaryFileSync } from "./boundary-file-read-CdxVvait.js";
...
import { a as redactSensitiveUrlLikeString } from "./redact-sensitive-url-Dj-KEs3a.js";
import { a as resolvePluginInstallDir, i as installPluginFromPath, r as installPluginFromNpmSpec, t as PLUGIN_INSTALL_ERROR_CODE } from "./install-Qx_NvgN3.js";
...
import { n as recordPluginInstall, t as buildNpmResolutionInstallFields } from "./installs-BAYVBHdj.js";
...
async function streamMarketplaceResponseToFile(params) {
	...
	const fileHandle = await fs$1.open(params.targetPath, "wx");
	let total = 0;
	try {
		while (true) {
...
async function downloadUrlToTempFile(url, timeoutMs) {
	...
	try {
		...
		try {
			...
			tmpDir = await fs$1.mkdtemp(path.join(os.tmpdir(), "openclaw-marketplace-download-"));
		...
		if (tmpDir) await fs$1.rm(tmpDir, {
			recursive: true,
			force: true
		}).catch(() => void 0);
		return {
...
async function ensureInsideMarketplaceRoot(rootDir, candidate, options) {
	...
	if (options?.canonicalRootDir) try {
		if (!(await fs$1.lstat(options.canonicalRootDir)).isDirectory()) throw new Error("invalid marketplace root");
		const rootRealPath = await fs$1.realpath(options.canonicalRootDir);
		...
		if (!isPathInside(rootRealPath, await fs$1.realpath(existingPath))) throw new Error("marketplace path escapes canonical root");
	} catch (error) {
...
async function validateMarketplaceManifest(params) {
	...
	const canonicalRootDir = await fs$1.realpath(params.rootDir);
...
async function resolveMarketplaceEntryInstallPath(params) {
	if (params.source.kind === "path") {
		...
		const canonicalRootDir = params.marketplaceOrigin === "remote" ? await fs$1.realpath(params.marketplaceRootDir) : void 0;
	...
	if (params.source.kind === "github" || params.source.kind === "git" || params.source.kind === "git-subdir") {
		...
		const canonicalRootDir = await fs$1.realpath(cloned.rootDir);
