...
import { C as resolveGroupSessionKey, b as deriveSessionMetaPatch, d as updateSessionStore, i as loadSessionStore } from "./store-CYkrTORt.js";
...
import { a as resolveSessionResetType, i as resolveSessionResetPolicy, n as evaluateSessionFreshness, o as resolveThreadFlag, r as resolveChannelResetConfig, t as resolveAndPersistSessionFile } from "./session-file-D4gyoUns.js";
import { n as resolveSessionKey } from "./session-key-B7z16Mey.js";
...
import { C as resolveFastModeState, D as getQueueSize, t as resolveQueueSettings, w as clearCommandLane } from "./queue-CbPppDLp.js";
...
import { n as parseInlineDirectives, t as isDirectiveOnly } from "./directive-handling.parse-BBSOPxmB.js";
...
import { n as resolveSessionAuthProfileOverride } from "./session-override-BQgdJCea.js";
import { n as resolveContextTokens, t as createModelSelectionState } from "./model-selection-ByVTyOJR.js";
...
function loadCommandsStatus() {
	commandsStatusPromise ??= import("./commands-status.runtime-BqkfJdkz.js");
...
function loadDirectiveImpl() {
	directiveImplPromise ??= import("./directive-handling.impl-IGrphcUN.js");
	return directiveImplPromise;
}
function loadDirectiveFastLane() {
	directiveFastLanePromise ??= import("./directive-handling.fast-lane-BqSCfSBX.js");
	return directiveFastLanePromise;
}
function loadDirectivePersist() {
	directivePersistPromise ??= import("./directive-handling.persist.runtime-BLB1aMIn.js");
...
function loadGroupsRuntime() {
	groupsRuntimePromise ??= import("./groups.runtime-BBKD1knr.js");
...
function loadCommandsRegistry() {
	commandsRegistryPromise ??= import("./commands-registry.runtime-DrrLV07k.js");
	return commandsRegistryPromise;
}
function loadSkillCommands() {
	skillCommandsPromise ??= import("./skill-commands.runtime-Cp-uznBX.js");
...
async function handleInlineActions(params) {
	...
	const skillCommands = shouldLoadSkillCommands && params.skillCommands ? params.skillCommands : shouldLoadSkillCommands ? (await import("./skill-commands.runtime-Cp-uznBX.js")).listSkillCommandsForWorkspace({
		workspaceDir,
		cfg,
		skillFilter
	}) : [];
	...
	if (skillInvocation) {
		...
		if (dispatch?.kind === "tool") {
			...
			const { createOpenClawTools } = await import("./openclaw-tools.runtime-ClnDo4O_.js");
	...
	if (!isAbortRequestText(command.rawBodyNormalized) && sessionEntry) {
		...
		if (cutoff) await (await import("./abort-cutoff.runtime-BCS7bSQn.js")).clearAbortCutoffInSessionRuntime({
			sessionEntry,
			sessionStore,
			sessionKey,
			storePath
	...
	if (handleInlineStatus) {
		const { buildStatusReply } = await import("./commands.runtime-CrIqfzmk.js");
	...
	const runCommands = async (commandInput) => {
		const { handleCommands } = await import("./commands.runtime-CrIqfzmk.js");
...
function loadSessionStoreRuntime$1() {
	sessionStoreRuntimePromise$1 ??= import("./store.runtime-DlO8U-zm.js");
...
function loadPiEmbeddedRuntime() {
	piEmbeddedRuntimePromise ??= import("./pi-embedded.runtime-rz8eUZnK.js");
	return piEmbeddedRuntimePromise;
}
function loadAgentRunnerRuntime() {
	agentRunnerRuntimePromise ??= import("./agent-runner.runtime-Dv0DdWHf.js");
	return agentRunnerRuntimePromise;
}
function loadRouteReplyRuntime() {
	routeReplyRuntimePromise ??= import("./route-reply.runtime-DbqgadM8.js");
	return routeReplyRuntimePromise;
}
function loadSessionUpdatesRuntime() {
	sessionUpdatesRuntimePromise ??= import("./session-updates.runtime-DDTupKdx.js");
	return sessionUpdatesRuntimePromise;
}
function loadSessionStoreRuntime() {
	sessionStoreRuntimePromise ??= import("./store.runtime-DlO8U-zm.js");
...
function loadDeliverRuntime() {
	deliverRuntimePromise ??= import("./deliver-runtime-r70S6v7G.js");
...
async function forkSessionFromParent(params) {
	return (await import("./session-fork.runtime-CBQ6zbWO.js")).forkSessionFromParentRuntime(params);
}
//#endregion
...
function loadSessionArchiveRuntime() {
	sessionArchiveRuntimePromise ??= import("./session-archive.runtime-_RF96x3-.js");
...
function loadSessionResetModelRuntime() {
	sessionResetModelRuntimePromise ??= import("./session-reset-model.runtime-R41DzWBd.js");
	return sessionResetModelRuntimePromise;
}
function loadStageSandboxMediaRuntime() {
	stageSandboxMediaRuntimePromise ??= import("./stage-sandbox-media.runtime-81RLBiGy.js");
...
async function applyMediaUnderstandingIfNeeded(params) {
	...
	const { applyMediaUnderstanding } = await import("./apply.runtime-CjYYhxT8.js");
	await applyMediaUnderstanding(params);
	return true;
}
async function applyLinkUnderstandingIfNeeded(params) {
	if (!hasLinkCandidate(params.ctx)) return false;
	const { applyLinkUnderstanding } = await import("./apply.runtime-CzVnsuwc.js");
...
async function getReplyFromConfig(ctx, opts, configOverride) {
	...
	const maybeEmitMissingResetHooks = async () => {
		...
		const { emitResetCommandHooks } = await import("./commands-core.runtime-B7VzAUyC.js");
