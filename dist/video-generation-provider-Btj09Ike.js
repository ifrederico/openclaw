...
async function pollXaiVideo(params) {
	for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
		...
		switch (payload.status) {
			...
			case "failed":
			case "expired": throw new Error(payload.error?.message?.trim() || `xAI video generation ${payload.status}`);
			default:
	...
	return {
		...
		async generateVideo(req) {
			...
			try {
				...
				if (!requestId) throw new Error(submitted.error?.message?.trim() || "xAI video generation response missing request_id");
