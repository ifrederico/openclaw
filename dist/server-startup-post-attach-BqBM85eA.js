...
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-Ce_dyoM5.js";
import { t as hasConfiguredInternalHooks } from "./configured-BbNgPWve.js";
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-Y__ud5Yd.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
async function refreshLatestUpdateRestartSentinelIfPresent() {
	...
	return await (await import("./server-restart-sentinel-BSa34btq.js")).refreshLatestUpdateRestartSentinel();
}
...
async function hasGatewayStartupInternalHookListeners() {
	const { hasInternalHookListeners } = await import("./internal-hooks-CAO3eGAT.js");
	return hasInternalHookListeners("gateway", "startup");
}
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-DTc3UdlZ.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	...
	const [{ resolveOpenClawAgentDir }, { resolveAgentWorkspaceDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }, { resolveEmbeddedAgentRuntime }] = await Promise.all([
		import("./agent-paths-CKvU1kbm.js"),
		import("./agent-scope-CaPKU--Z.js"),
		import("./defaults-C0MyiUmB.js"),
		import("./model-selection-D-Nmv1t8.js"),
		import("./runtime-T2BczB7I.js")
	]);
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-Ch5Geid8.js");
...
async function startGatewaySidecars(params) {
	await measureStartup(params.startupTrace, "sidecars.gmail-watch", async () => {
		if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-CotyLjG7.js");
	...
	await measureStartup(params.startupTrace, "sidecars.gmail-model", async () => {
		if (params.cfg.hooks?.gmail?.model) {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				import("./defaults-C0MyiUmB.js"),
				import("./model-catalog-DRHwiBpJ.js"),
				import("./model-selection-D-Nmv1t8.js")
			]);
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([import("./internal-hooks-CAO3eGAT.js"), import("./loader-CX14-BOr.js")]);
	...
	if (internalHooksConfigured || await hasGatewayStartupInternalHookListeners()) setTimeout(() => {
		import("./internal-hooks-CAO3eGAT.js").then(({ createInternalHookEvent, triggerInternalHook }) => {
	...
	await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-DqAbDrlq.js");
	...
	if (params.cfg.acp?.enabled) (async () => {
		...
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./manager-BdjCGWbR.js"), import("./session-identifiers-DFZ16nTQ.js")]);
		...
		run: async () => {
			try {
				const [{ resolveStateDir }, { resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([
					...
					import("./session-dirs-d3j8wTa5.js"),
					import("./session-write-lock-qa9NiHvn.js")
				]);
				...
				for (const sessionsDir of sessionDirs) {
					...
					if (result.cleaned.length > 0) {
						const { markRestartAbortedMainSessionsFromLocks } = await import("./main-session-restart-recovery-BFCFyp7I.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async () => {
			...
			setTimeout(() => {
				import("./server-restart-sentinel-BSa34btq.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-DF5PMP9O.js");
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleRestartAbortedMainSessionRecovery } = await import("./main-session-restart-recovery-BFCFyp7I.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
	getGlobalHookRunner: async () => (await import("./hook-runner-global-BMiiOjOf.js")).getGlobalHookRunner(),
	logGatewayStartup: async (params) => (await import("./server-startup-log-DhGhIF0e.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-C-h0OOVB.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-DsvbiQEs.js")).startGatewayTailscaleExposure(...args)
};
