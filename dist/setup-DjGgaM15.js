...
import { d as normalizeSecretInputString } from "./types.secrets-CL51SR4g.js";
import { n as defaultRuntime } from "./runtime-bzt9CHmD.js";
import { r as normalizeProviderId } from "./provider-id-DIRgKpoh.js";
import { r as createConfigIO } from "./io-E69J4lLI.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-DEJL5ql6.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-CIDM9iue.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-CH-sN9x8.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-Dx6Bpz-W.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-BNdURonc.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-D-zBfKcM.js"), import("./setup-registry-XfZrXVvS.js")]);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-pdUi8aoB.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-B3ZfjVC7.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-Ciohi0l0.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-Ciohi0l0.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-BkEc919f.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-B0h45gEX.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-C8j0PAkz.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-DSzutJtm.js");
		const { setupChannels } = await import("./onboard-channels-Cn9rgzW6.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-DyeR0e7F.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DBc5jdmR.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-BG6OZ4Ux.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-CLDOwsCW.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-CPZlHb3j.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-DVm2cGuJ.js");
