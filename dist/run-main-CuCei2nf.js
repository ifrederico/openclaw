...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-CcxK7Mxo.js");
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		if (timeoutMs === null) return false;
		if (json) {
			const { statusJsonCommand } = await import("./status-json-CIVyb2T3.js");
		...
		const { statusCommand } = await import("./status-C0nqWe6D.js");
		await statusCommand({
			json,
			deep,
			all,
...
const routeGatewayStatus = {
	...
	run: async (argv) => {
		...
		const { runDaemonStatus } = await import("./status-C_3LbXY2.js");
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DitmM6Um.js");
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-Cm_8mxKD.js");
...
const routes = [
	...
	routeAgentsList,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-Y536hiWN.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			...
			const pathArg = positionals[0];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-Y536hiWN.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-CNwtJl1c.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-CNwtJl1c.js");
...
async function prepareRoutedCommand(params) {
	...
	if (!suppressDoctorStdout && process.stdout.isTTY) {
		const [{ emitCliBanner }, { VERSION }] = await Promise.all([import("./banner-DUbBjd81.js"), import("./version-D6Q9DSv_.js")]);
		emitCliBanner(VERSION, { argv: params.argv });
	}
	if (!skipConfigGuard) {
		const { ensureConfigReady } = await import("./config-guard-DwLcY64O.js");
	...
	if (typeof params.loadPlugins === "function" ? params.loadPlugins(params.argv) : params.loadPlugins) {
		const { ensurePluginRegistryLoaded } = await import("./plugin-registry-BKO02-Kj.js");
...
async function runCli(argv = process$1.argv) {
	...
	if (shouldLoadCliDotEnv()) {
		const { loadCliDotEnv } = await import("./dotenv-BQPu01x0.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			const { outputPrecomputedRootHelpText } = await import("./root-help-metadata-R8Nn5KNd.js");
			if (!outputPrecomputedRootHelpText()) {
				const { outputRootHelp } = await import("./root-help-iHCc_H4e.js");
		...
		const { buildProgram } = await import("./program-B_CMvaXk.js");
		...
		if (primary) {
			const { getProgramContext } = await import("./program-context-BIK4Dqeq.js");
			const ctx = getProgramContext(program);
			if (ctx) {
				const { registerCoreCliByName } = await import("./command-registry-BauZFng8.js");
				await registerCoreCliByName(program, ctx, primary, parseArgv);
			}
			const { registerSubCliByName } = await import("./register.subclis-CW2Sqj8l.js");
		...
		if (!shouldSkipPluginCommandRegistration({
			...
			const { registerPluginCliCommands } = await import("./cli-DaXxyufu.js");
			const { loadValidatedConfigForPluginRegistration } = await import("./register.subclis-CW2Sqj8l.js");
