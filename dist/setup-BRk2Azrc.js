...
import { h as writeConfigFile, l as readConfigFileSnapshot } from "./io-CS2J_l4V.js";
...
import { n as buildPluginCompatibilityNotices, s as formatPluginCompatibilityNotice } from "./status-BC4s-4i3.js";
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const { resolvePluginProviders, resolveProviderPluginChoice } = await import("./provider-auth-choice.runtime-BngMur4l.js");
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-BuUkviDY.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-gXcUfnkM.js");
		const { logConfigUpdated } = await import("./logging-BDhb5XFW.js");
	...
	const { applyLocalSetupWorkspaceConfig } = await import("./onboard-config-DXCtfMb8.js");
	let nextConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
	const { ensureAuthProfileStore } = await import("./agents/auth-profiles.runtime.js");
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-DYuACGTg.js");
	const { promptCustomApiConfig } = await import("./onboard-custom-vp1dLu_e.js");
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-BRYLQY5Q.js");
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-Cje-HRxb.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-uVdvjc1c.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-J8xdlrOz.js");
		const { setupChannels } = await import("./onboard-channels-Bx4N8THE.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-CyJUB0oM.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DHskzGXU.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupPluginConfig } = await import("./setup.plugin-config-CqjiNY9Q.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-ODq8qGtv.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-ChEuvywz.js");
