...
async function fetchInferenceProfileSummaries(client) {
	try {
		...
		log.debug?.("Skipping inference profile discovery", { error: error instanceof Error ? error.message : String(error) });
		return [];
	}
}
/**
...
function resolveInferenceProfiles(profiles, defaults, providerFilter, foundationModels) {
	...
	for (const profile of profiles) {
		...
		const baseModel = baseModelId ? foundationModels.get(baseModelId.toLowerCase()) : void 0;
		discovered.push({
...
async function discoverBedrockModels(params) {
	...
	const discoveryPromise = (async () => {
		...
		for (const profile of inferenceProfiles) if (!seenIds.has(profile.id.toLowerCase())) {
