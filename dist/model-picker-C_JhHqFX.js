...
import { c as modelKey, i as buildModelAliasIndex, m as resolveConfiguredModelRef, t as buildAllowedModelSet } from "./model-selection-BVM4eHHo.js";
...
import { i as formatTokenK } from "./shared-Dpgo97-6.js";
...
async function loadModelPickerRuntime() {
	return import("./model-picker.runtime-BGdE34us.js");
}
...
async function maybeFilterModelsByProvider(params) {
	...
	if (hasPreferredProvider && params.preferredProvider) {
		const filtered = next.filter((entry) => matchesPreferredProvider(entry.provider, params.preferredProvider));
...
async function promptDefaultModel(params) {
	...
	if (allowKeep && hasPreferredProvider && preferredProvider && !matchesPreferredProvider(resolved.provider, preferredProvider)) {
