...
import { t as require_src$1 } from "./src-hV2aBDoy.js";
import { t as require_jsonwebtoken } from "./jsonwebtoken-BBQNhIl7.js";
import { n as require_logging, t as require_http } from "./http-BH65ftYP.js";
		...
		*/
		async validateAccessToken(rawToken, overrideOptions) {
			...
			return new Promise((resolve) => {
				const verifyOptions = {
					audience: [this.options.clientId, `api://${this.options.clientId}`],
					issuer: void 0,
		...
		validateIssuer(iss) {
			if (!this.options.validateIssuer) return;
			if (!iss) throw new Error("Token missing issuer claim");
			if ("allowedIssuer" in this.options.validateIssuer) {
				if (iss !== this.options.validateIssuer.allowedIssuer) throw new Error(`Token issuer '${iss}' does not match allowed issuer '${this.options.validateIssuer.allowedIssuer}'`);
				return;
			}
			...
			if (isMultiTenant) {
				if (this.options.validateIssuer.allowedTenantIds) {
					for (const tenantId of this.options.validateIssuer.allowedTenantIds) if (![
						"common",
						"organizations",
						"consumers"
			...
			else if (!allowedTenantIds.some((tenantId) => iss.startsWith(`https://login.microsoftonline.com/${tenantId}/`))) throw new Error(`Token issuer '${iss}' not in allowed tenant IDs: ${allowedTenantIds.join(", ")}`);
		}
...
//#endregion
export { require_events as a, require_storage as i, require_asserts as n, require_dist as r, require_jwt_validator as t };
