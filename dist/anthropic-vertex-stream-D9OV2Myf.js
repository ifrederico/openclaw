...
var APIError$1 = class APIError$1 extends AnthropicError {
	...
	static generate(status, errorResponse, message, headers) {
		...
		if (status === 400) return new BadRequestError$1(status, error, message, headers);
		if (status === 401) return new AuthenticationError$1(status, error, message, headers);
		if (status === 403) return new PermissionDeniedError$1(status, error, message, headers);
		if (status === 404) return new NotFoundError$1(status, error, message, headers);
		if (status === 409) return new ConflictError$1(status, error, message, headers);
		if (status === 422) return new UnprocessableEntityError$1(status, error, message, headers);
		if (status === 429) return new RateLimitError$1(status, error, message, headers);
		if (status >= 500) return new InternalServerError$1(status, error, message, headers);
		return new APIError$1(status, error, message, headers);
	}
};
		...
		async function* iterator() {
			...
			try {
				for await (const sse of _iterSSEMessages$1(response, controller)) {
					...
					if (sse.event === "error") throw new APIError$1(void 0, safeJSON$1(sse.data) ?? sse.data, void 0, response.headers);
				}
				done = true;
			} catch (e) {
