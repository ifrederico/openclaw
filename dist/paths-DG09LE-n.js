...
import { n as isCompactionCheckpointTranscriptFileName } from "./artifacts-CL4aKqhQ.js";
...
function resolveRotatedGeneratedSessionFilePath(params) {
	...
	try {
		...
		if (!path.isAbsolute(previousSessionFile)) return;
		const relative = path.relative(path.resolve(params.sessionsDir), path.resolve(previousSessionFile));
		if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) return;
	}
...
//#endregion
export { resolveSessionFilePath as a, resolveSessionTranscriptPathInDir as c, resolveStorePath as d, validateSessionId as f, resolveRotatedGeneratedSessionFilePath as i, resolveSessionTranscriptsDir as l, resolveAgentsDirFromSessionStorePath as n, resolveSessionFilePathOptions as o, resolveDefaultSessionStorePath as r, resolveSessionTranscriptPath as s, SAFE_SESSION_ID_RE as t, resolveSessionTranscriptsDirForAgent as u };
